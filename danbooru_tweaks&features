...
function $q(url, callback, method, asyn) {
  ...
  xmlhttp.onreadystatechange = function () {
    if (xmlhttp.readyState == 4) {
      if (method != 'HEAD') callback(xmlhttp.responseText);
	    else callback(xmlhttp.getAllResponseHeaders());
    }
  };
...
function applyCustomCSS() {
  ...
  */
  ...
  'div#c-posts div#a-index div#posts div:first-child { overflow:visible!important; }'+ //minor fix so endlesspages fits in well
  ...
  '#nav-links { margin:0.3em 0em!important; padding: 0.3em!important; }'+    //resize of the pools/search nav box - (from 1em/0.5em)
...
function settings_init() {
  ...
  if(settings_jsonData) { //data, not settings
    try {
  ...
  try {
    for (i = 0; i < comments.length; i++) {
      ...
      input.onclick = function () {
        ...
        $x("../li[last()-2]", this)[0].style.display = 'inline';
        this.style.display = 'none';
      };
      $x("//div[@class='content']/menu/li[last()]")[i].style.display = 'none';
      $x("//div[@class='content']/menu/li[last()-1]")[i].style.display = 'none';
      $x("//div[@class='content']/menu/li[last()-2]")[i].style.display = 'none';
      $x("//div[@class='content']/menu")[i].insertBefore(input, $x("//div[@class='content']/menu/li[last()-2]")[i]);
    }
  }
  catch(e) {
...
function endlessPages_init() {
  ...
  if(ep_onPostsListPage && loc.indexOf("limit=")!=-1) {
    ...
    ep_scrollTrigger = ep_scrollTrigger + ((ep_limit-20)*7); //pages with &limit take a lot longer to load, increase distance at which it starts preloading
  }
  //init pools
...
function ep_postsList(add) {
  if (add == undefined && (!ep_lockQuerry)) { //no value given, query next page of posts (which upon completion calls this function again with the json result)
    ...
    for (post in add) {
      ...
      if (add[post].md5 == undefined) continue; //some posts (banned, deleted, etc) don't have md5 and can't be displayed, data is still in API for other purposes
      if (add[post].is_flagged) ep_postClass += ' post-status-flagged';
      if (add[post].is_pending) ep_postClass += ' post-status-pending';
      if (add[post].has_children) ep_postClass += ' post-status-has-children';
      if (add[post].parent_id) ep_postClass += ' post-status-has-parent';
      var art = $c("article", {
      ...
      var art_a_img = $c("img", {
        src: add[post].preview_file_url, //new attribute, don't have to manually add "/data/preview/" to the md5 anymore
  ...
  if (add == undefined && (!ep_lockQuerry)) {
    ...
    for (i1 = 0; i1 < ep_poolsToQueryList.length; i1++) {
     t2=null;
      ...
      if (add[t2].md5 == undefined) continue; //some posts (banned, deleted, etc) don't have md5 and can't be displayed, data is still in API for other purposes
      ...
      if (add[t2].is_pending) ep_postClass += ' post-status-pending';
      if (add[t2].has_children) ep_postClass += ' post-status-has-children';
      if (add[t2].parent_id) ep_postClass += ' post-status-has-parent';
      var art = $c("article", {
        class: "post-preview" + ep_postClass,
        id: "post_" + add[t2].id,
        "data-id": add[t2].id,
      ...
      var art_a_img = $c("img", {
        src: add[t2].preview_file_url, //new attribute, don't have to manually add "/data/preview/" to the md5 anymore
...
function ep_browsePools_getList(list, isPool) {
  if (isPool && !ep_browsePools_list) { //reacts to button press
    ep_browsePools_poolID = list;

    var temp_browseButtons = document.getElementsByClassName('browsePool');
    for (i = temp_browseButtons.length - 1; i >= 0; i--) { //remove other 'browse' buttons, can only browse 1 pool at a time
...
function ep_browsePools(post) {
  ...
  if (!post && !ep_lockQuerry) {
    ...
    temp_id = temp_doc.getElementsByName('post-id')[0].content;  //id of the image we just received and are adding to the page
    
    var temp_image_container = temp_doc.getElementById('image-container');
    var temp_image_direct = temp_doc.getElementById('image');
    
    if(!temp_image_direct) { //no image found, retrieve some data & skip   (flash/deleted/etc)
      var temp_ep_container_ = $c("div", { id: 'ep_container_' + temp_id, class: 'ep-browse-main', style: 'white-space:nowrap' });
      temp_ep_container_.innerHTML = temp_ep_container_.innerHTML + '<a href="/posts/'+temp_id+'" style="font-weight:bold">No image found here at Post #'+temp_id+' - Possible reason:</a>';
    ...
    edit_tools.innerHTML=
      ...
      '<textarea id="newTags_'+temp_id+'" rows="5" type="text" style="width:26em;">'+tagString+'</textarea>'+
...
function customBlacklist_init() {
  ...
  //else customBlacklist_tags='rating:e,rating:q,nipples,areola_slip,bdsm,pussy,naked_ribbon,naked_apron,naked_towel,bandaid_on_pussy,bikini';//defaults
...
function customBlacklist(s) {
  ...
  if(s=='setup') { //setup - runs at pageload
    ...
    if(s.shiftKey) { //shift-key
      ...
      if(newTags==='default' || newTags==='') newTags = 'rating:e,rating:q,nipples,areola_slip,bdsm,pussy,naked_ribbon,naked_apron,naked_towel,bandaid_on_pussy,bikini';//reset to default
